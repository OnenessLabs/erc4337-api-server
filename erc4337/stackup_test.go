...
func noTestCheckUserOp(t *testing.T) {
	nodeRpc, err := rpc.Dial(os.Getenv("ERC4337_API_BUNDLER_URL"))
...
func noTestSendUserOp(t *testing.T) {
	...
	pmRpc, err := rpc.Dial(os.Getenv("ERC4337_API_PAYMASTER_URL"))
	require.NoError(t, err)
...
func noTestOpSubmitDirect(t *testing.T) {
	...
	sk, err := crypto.SKFromHex(os.Getenv("ERC4337_API_ETH_CLIENT_SK"))
	require.NoError(t, err)
	k := &chain.EcdsaKey{SK: sk}
	println(k.Address().String())

	ec, err := jsonrpc.NewClient(os.Getenv("ERC4337_API_ETH_CLIENT_URL"))
	require.NoError(t, err)
...
func noTestStackupClient(t *testing.T) {
	...
	conf := struct {
		...
		EthClientUrl:            os.Getenv("ERC4337_API_ETH_CLIENT_URL"),
